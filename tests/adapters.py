...
def run_swiglu(
    ...
    swiglu = SwiGLU(d_model, d_ff, bias=False)
...
def run_multihead_self_attention(
    ...
    *batch_dims, seq_len, d_in = in_features.shape
    ...
    # Apply Q, K, V projections directly using matrix multiplication
    # The weights are (d_model, d_in), so we get (batch, seq_len, d_model) outputs
    q = torch.matmul(in_features, q_proj_weight.T)  # (..., seq_len, d_model)
    k = torch.matmul(in_features, k_proj_weight.T)
    v = torch.matmul(in_features, v_proj_weight.T)
    
    # Reshape for multi-head attention: (batch, seq_len, d_model) -> (batch, seq_len, num_heads, d_k) -> (batch, num_heads, seq_len, d_k)
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    from cs336_basics.model import scaled_dot_product_attention, Linear, rope
    
    *batch_dims, seq_len, d_in = in_features.shape
    d_k = q_proj_weight.shape[0] // num_heads
    
    # Create Q, K, V projections
    q_proj = Linear(d_in, d_model, bias=False)
    k_proj = Linear(d_in, d_model, bias=False) 
    v_proj = Linear(d_in, d_model, bias=False)
    o_proj = Linear(d_model, d_model, bias=False)
    
    # Set weights - reshape from per-head to full model dimension
    q_proj.weight.data = q_proj_weight.view(num_heads, d_k, d_in).transpose(0, 1).contiguous().view(d_model, d_in)
    k_proj.weight.data = k_proj_weight.view(num_heads, d_k, d_in).transpose(0, 1).contiguous().view(d_model, d_in)
    v_proj.weight.data = v_proj_weight.view(num_heads, d_k, d_in).transpose(0, 1).contiguous().view(d_model, d_in)
    o_proj.weight.data = o_proj_weight
    
    # Project to Q, K, V
    q = q_proj(in_features)  # (..., seq_len, d_model)
...
def run_rope(
    ...
    return rope(in_query_or_key, theta)
...
def run_transformer_block(
    ...
    block.feedforward.swiglu.w1.weight.data = weights['ffn.w1.weight']
    block.feedforward.swiglu.w2.weight.data = weights['ffn.w2.weight']
    block.feedforward.swiglu.w3.weight.data = weights['ffn.w3.weight']
